...
prueba7[, 5] <- gsub("[^[:alnum:][:space:]]", "", prueba6[, 5]) # quitar non-word characters # nolint: line_length_linter.
prueba8 <- drop_na(prueba7)

prueba9 <- prueba8[, 4:5]   # sólo año y texto

prueba10 <- prueba9 %>%
  group_by(año) %>%
  summarize(texto = paste(texto, collapse = ""))

textos <- prueba10$texto
